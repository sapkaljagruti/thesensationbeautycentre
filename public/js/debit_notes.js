...
$(document).on('change', '.select-all', function (e) {
    ...
    $('input[name="select_debit_notes[]"]').each(function () {
        this.checked = status;
    });
    $('#delete_selected').attr('disabled', !(status));
});
$(document).on('change', 'input[name="select_debit_notes[]"]', function (e) {
    if (this.checked == false) {
        $(".select-all")[0].checked = false;
    }

    if ($('input[name="select_debit_notes[]"]:checked').length > 0) {
...
$(document).on('click', '#delete', function () {
    ...
    if (delete_type == 'single') {
        ...
        var data_to_delete = [];
        $('input[name="select_debit_notes[]"]:checked').each(function () {
...
$('#ledger_name').autocomplete({
    source: function (request, response) {
        $.ajax({
            url: '?controller=purchase&action=findLedgerByTerm',
            type: "POST",
            dataType: "json",
            data: {
                term: request.term,
    ...
    select: function (event, ui) {
        $('#purchase_voucher_id').val(ui.item.id);
        $('#purchase_party_id').val(ui.item.party_id);
    }
});
...
$(document).on('keyup', '#ledger_name', function () {
    if ($(this).val() == '') {
        ...
        $.ajax({
            url: '?controller=purchase&action=checkLedgerNameExist',
            data: {
...
function enterKeyEvent(e) {
    if (e.keyCode == 13) {
        var purchase_voucher_id = $.trim($('#purchase_voucher_id').val());
...
$(document).on('click', '#proceed_particular', function () {
    ...
    if (ledger_name == '') {
        ...
        if (purchase_voucher_id == '') {
            $('#ledger_name').css('border-color', '#dd4b39');
            $('#ledger_name_label').css('color', '#dd4b39');
            $('#ledger_name_help_block').html('<font color="#dd4b39">Purchase Ledger Not Found</font>');
            proceed = 0;
        } else {
    ...
    if (proceed != 0) {
        ...
        if ($('#tr_' + purchase_voucher_id).length) {
            var old_amount = $('#tr_' + purchase_voucher_id + ' td:nth-child(2)').text();
        ...
        if ($('#tr_' + purchase_voucher_id).length) {
            ...
            table.cell('#tr_' + purchase_voucher_id, ':eq(1)').data(ledger_name).draw();
            table.cell('#tr_' + purchase_voucher_id, ':eq(2)').data(new_amount).draw();
            $('#tr_' + purchase_voucher_id).attr('data-amount', new_amount);
        } else {
            var table = $('#particulars_table').DataTable();
            var rowNode = table.row.add([purchase_voucher_id, ledger_name, new_amount, '<a href="" class="delete_particular" data-id="' + purchase_voucher_id + '"> <i class="fa fa-fw fa-trash"></i></a>']).draw().node();
            $(rowNode).attr('id', 'tr_' + purchase_voucher_id);
            $(rowNode).attr('data-id', purchase_voucher_id);
...
$(document).on('focusout', '#debit_note_no', function () {
    var debit_note_no = $('#debit_note_no').val();

    if ($.trim(debit_note_no) == '') {
        $('#valid_debit_note_no').val('0');
        $('#debit_note_no').css('border-color', '#dd4b39');
        $('#debit_note_no_label').css('color', '#dd4b39');
        $('#debit_note_no_help_block').html('<font color="#dd4b39">Please enter Debit Note No</font>');
    } else {
        $('#overlay_invoice').show();
        $('#overlay_particular').show();
        $('#overlay_party').show();
        $.ajax({
            url: '?controller=debitnotes&action=checkDebitNoteExist',
            data: {
            ...
            success: function (response) {
                ...
                if (response == '1') {
                    ...
                    $('#debit_note_no').css('border-color', '#dd4b39');
                    $('#debit_note_no_label').css('color', '#dd4b39');
                    $('#debit_note_no_help_block').html('<font color="#dd4b39">This debit note already exists.</font>');
                } else {
                    $('#valid_debit_note_no').val('1');
                    $('#debit_note_no').css('border-color', 'rgb(210, 214, 222)');
                    $('#debit_note_no_label').css('color', 'black');
                    $('#debit_note_no_help_block').html('');
                }
            },
            error: function (xhr, status, error) {
                showError('Something went wrong. Please try again later.', 5000);
...
$(document).on('submit', 'form', function () {
    ...
    var valid_debit_note_no = $('#valid_debit_note_no').val();

    if (valid_debit_note_no == '0') {
        proceed = 0;
    }
    ...
    if (!($('.particulars').length)) {
        ...
        $('.particulars').each(function () {
            ...
            purchase_invoice_data.push(data_id + '_' + data_amount);
        });
        $('#purchase_invoice_data').val(purchase_invoice_data);
    }
...
$(function () {
    ...
    $('#delete_selected').attr('disabled', true);
    var t = $('#debit_notes_table').DataTable({
        "paging": true,
        "lengthChange": true,
