...
$('#ledger_name').autocomplete({
    source: function (request, response) {
        $.ajax({
            url: '?controller=accountgroup&action=findPurchaseLedgerByTerm',
            type: "POST",
            dataType: "json",
            data: {
                term: request.term,
    ...
    select: function (event, ui) {
        $('#purchase_ledger_id').val(ui.item.id);
    }
});
...
$(document).on('keyup', '#ledger_name', function () {
    if ($(this).val() == '') {
        ...
        var purchase_ledger_id = $('#purchase_ledger_id').val();
        $('#overlay_particular').show();
        $.ajax({
...
function enterKeyEvent(e) {
    if (e.keyCode == 13) {
        var purchase_ledger_id = $.trim($('#purchase_ledger_id').val());
...
$(document).on('click', '#proceed_particular', function () {
    ...
    if (proceed != 0) {
        ...
        if ($('#tr_' + purchase_ledger_id).length) {
            var old_amount = $('#tr_' + purchase_ledger_id + ' td:nth-child(2)').text();
        ...
        if ($('#tr_' + purchase_ledger_id).length) {
            ...
            table.cell('#tr_' + purchase_ledger_id, ':eq(1)').data(ledger_name).draw();
            table.cell('#tr_' + purchase_ledger_id, ':eq(2)').data(new_amount).draw();
            $('#tr_' + purchase_ledger_id).attr('data-amount', new_amount);
        } else {
            var table = $('#particulars_table').DataTable();
            var rowNode = table.row.add([purchase_ledger_id, ledger_name, new_amount, '<a href="" class="delete_particular" data-id="' + purchase_ledger_id + '"> <i class="fa fa-fw fa-trash"></i></a>']).draw().node();
            $(rowNode).attr('id', 'tr_' + purchase_ledger_id);
            $(rowNode).attr('data-id', purchase_ledger_id);
