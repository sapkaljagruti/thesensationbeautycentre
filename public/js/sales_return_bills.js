...
$(document).on('change', '.select-all', function (e) {
    ...
    $('input[name="select_sales_return_vouchers[]"]').each(function () {
        this.checked = status;
    });
    $('#delete_selected').attr('disabled', !(status));
});

$(document).on('change', 'input[name="select_sales_return_vouchers[]"]', function (e) {
    if (this.checked == false) {
        $(".select-all")[0].checked = false;
    }

    if ($('input[name="select_sales_return_vouchers[]"]:checked').length > 0) {
...
$(document).on('click', '#delete', function () {
    ...
    if (delete_type == 'single') {
        ...
        var data_to_delete = [];
        $('input[name="select_sales_return_vouchers[]"]:checked').each(function () {
...
$(document).on('focusout', '#invoice_no', function () {
    ...
    if ($.trim(invoice_no) == '') {
        ...
        $('#overlay_party').show();
        $.ajax({
            url: '?controller=salesreturn&action=checkInovieExist',
            data: {
                'id': id,
...
$(document).on('click', '#proceed_sales_bill', function () {
    var proceed = 1;

    var party_id = $('#party_id').val();
    var sales_invoice_no = $.trim($('#sales_invoice_no').val());
    var sales_invoice_date_range = $.trim($('#sales_invoice_date_range').val());
    ...
    if (sales_invoice_no == '' && sales_invoice_date_range == '') {
        $('#sales_bill_error').html('<font color="#dd4b39">Please enter invoice no or invoice date range.</font>');
        $('#sales_bill_error_div').show();
        proceed = 0;
    } else {
        $('#sales_bill_error').html('');
        $('#sales_bill_error_div').hide();
    }

    if (proceed != 0) {
        $('.overlay').show();
        $.ajax({
            url: '?controller=sale&action=filterBillForReturn',
            data: {
                'party_id': party_id,
                'sales_invoice_no': sales_invoice_no,
                'sales_invoice_date_range': sales_invoice_date_range
            },
            type: 'post',
            dataType: "json",
            success: function (response) {
                if (response.length) {
                    ...
                    $('#s_invoice_no').html('<option value="">Select Invoice No</option>');
                    $('#product_id').html('<option value="">Select Product</option>');
                    $('#sales_voucher_id').val('');

                    $.each(response, function (item, obj) {
                        $('#s_invoice_no').append($("<option></option>").attr("value", obj.invoice_no).text(obj.invoice_no));
                    });
                } else {
                    $('#sales_bill_error').html('<font color="#dd4b39">No data found.</font>');
                    $('#sales_bill_error_div').show();
...
$(document).on('change', '#s_invoice_no', function () {
    var s_invoice_no = $(this).val();
    if (s_invoice_no != '') {
        $('.overlay').show();
        $.ajax({
            url: '?controller=sale&action=getFromInvoiceNo',
            data: {
                ...
                if (response.length) {
                    ...
                    $.each(response, function (item, obj) {
                        ...
                        for (var i in products_data_arr) {
                            ...
                            var price = product_arr[4];
                            var qty = product_arr[5];
                            var final_updated_qty = product_arr[6];
                            var discount_percentage = product_arr[7];
                            var discount_rs = product_arr[8];
                            var cgst_percentage = product_arr[9];
                            var cgst_rs = product_arr[10];
                            var sgst_percentage = product_arr[11];
                            var sgst_rs = product_arr[12];
                            var igst_percentage = product_arr[13];
                            var igst_rs = product_arr[14];
                            var total_amount = product_arr[15];
...
$(document).on('change', '#product_id', function () {
    ...
    if (product_id != '') {
        ...
        $('#sales_qty').val(selected_product.attr('data-qty'));
    } else {
        $('#qty').val('');
        $('#sales_qty').val('');
    }
...
$(document).on('click', '#proceed_product', function () {
    ...
    if (quantity == '') {
        ...
        if (quantity == '') {
            ...
            if (quantity > sales_qty) {
                $('#qty').css('border-color', '#dd4b39');
                $('#qty_help_block').html('<font color="#dd4b39">Quantity should be less than quantity you sold.</font>');
                proceed = 0;
            } else {
    ...
    if (proceed != 0) {
        ...
        if ($('#tr_' + product_id).length) {
            ...
            var rowNode = table.row.add([product_id, target_account, product_name, hsn_code, quantity, price, discount_percentage, discount_rs, cgst_percentage, cgst_rs, sgst_percentage, sgst_rs, igst_percentage, igst_rs, total_amount, '<a href="" class="delete_product" data-id="' + product_id + '"> <i class="fa fa-fw fa-trash"></i></a>']).draw().node();
...
$(document).on('submit', 'form', function () {
    ...
    if (!($('.products').length)) {
        ...
        $('.products').each(function () {
            ...
            product_details.push(data_id + '_' + data_tid + '_' + data_name + '_' + data_hcode + '_' + data_price + '_' + data_qty + '_' + data_dper + '_' + data_drs + '_' + data_cgstper + '_' + data_cgstrs + '_' + data_sgstper + '_' + data_sgstrs + '_' + data_igstper + '_' + data_igstrs + '_' + data_total);
        });
...
//    Date range picker
    $('#sales_invoice_date_range').daterangepicker({
        locale: {
            format: 'DD/MM/YYYY'
        },
    ...
    var t = $('#sales_return_vouchers_table').DataTable({
        "paging": true,
        "lengthChange": true,
