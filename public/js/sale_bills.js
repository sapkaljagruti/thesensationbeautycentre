...
$(document).on('change', '.select-all', function (e) {
    ...
    $('input[name="select_sales_vouchers[]"]').each(function () {
        this.checked = status;
    });
    $('#delete_selected').attr('disabled', !(status));
});

$(document).on('change', 'input[name="select_sales_vouchers[]"]', function (e) {
    if (this.checked == false) {
        $(".select-all")[0].checked = false;
    }

    if ($('input[name="select_sales_vouchers[]"]:checked').length > 0) {
...
$(document).on('click', '#delete', function () {
    ...
    if (delete_type == 'single') {
        ...
        $('#loader').show();
        $.ajax({
            url: '?controller=sale&action=delete',
            type: "POST",
        ...
        var data_to_delete = [];
        $('input[name="select_sales_vouchers[]"]:checked').each(function () {
...
$(document).on('change', '#sales_type_id', function () {
    var sales_type = $(this).val();

    if (sales_type == '1') { //Interstate Sale
        ...
        $('#sgst').val('');
        $('#igst').val('');
    } else if (sales_type == '2') { //Local Sale        
        ...
        $.ajax({
            ...
            success: function (finalQty) {
                ...
                if ($('#tr_' + product_id).length) {
                    ...
                    var rowNode = table.row.add([product_id, target_account, product_name + '</br>' + '<font color="red">' + finalQty + ' Nos in stock.', hsn_code, quantity, mrp, discount_percentage, discount_rs, cgst_percentage, cgst_rs, sgst_percentage, sgst_rs, igst_percentage, igst_rs, total_amount, '<a href="" class="delete_product" data-id="' + product_id + '"> <i class="fa fa-fw fa-trash"></i></a>']).draw().node();
...
$(function () {
    ...
    var t = $('#sales_vouchers_table').DataTable({
        "paging": true,
        "lengthChange": true,
...
//    if (!($("tr[id^='tr_']").length)) {
    if (!($('.products').length)) {
        ...
        $('.products').each(function () {
            ...
            product_details.push(data_id + '_' + data_tid + '_' + data_name + '_' + data_hcode + '_' + data_mrp + '_' + data_qty + '_' + data_finalQty + '_' + data_dper + '_' + data_drs + '_' + data_cgstper + '_' + data_cgstrs + '_' + data_sgstper + '_' + data_sgstrs + '_' + data_igstper + '_' + data_igstrs + '_' + data_total);
        });
