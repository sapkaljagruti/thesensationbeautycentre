...
$(document).on('change', '.select-all', function (e) {
    ...
    $('input[name="select_journal_vouchers[]"]').each(function () {
        this.checked = status;
    });
    $('#delete_selected').attr('disabled', !(status));
});

$(document).on('change', 'input[name="select_journal_vouchers[]"]', function (e) {
    if (this.checked == false) {
        $(".select-all")[0].checked = false;
    }

    if ($('input[name="select_journal_vouchers[]"]:checked').length > 0) {
...
$(document).on('click', '#delete', function () {
    ...
    if (delete_type == 'single') {
        ...
        $.ajax({
            ...
            success: function (response) {
                if ($.trim(response) == 'deleted') {
                    var table = $('#journal_vouchers_table').DataTable();
        ...
        var data_to_delete = [];
        $('input[name="select_journal_vouchers[]"]:checked').each(function () {
...
$(function () {
    ...
    var t = $('#journal_vouchers_table').DataTable({
        "paging": true,
        "lengthChange": true,
...
$('#ledger_name').autocomplete({
    source: function (request, response) {
        $.ajax({
            url: '?controller=journal&action=getAllAccounts',
            type: "POST",
            dataType: "json",
            data: {
                term: request.term,
...
$(document).on('keyup', '#ledger_name', function () {
    if ($(this).val() == '') {
        ...
        $.ajax({
            url: '?controller=journal&action=checkLedgerNameExist',
            data: {
...
function enterKeyEvent(e) {
    if (e.keyCode == 13) {
        var ledger_id = $.trim($('#ledger_id').val());
        if (ledger_id == '') {
            $('#ledger_name').val('');
            $('#amount').val('');
        }
...
$(document).on('click', '#proceed_particular', function () {
    ...
    if (amount == '') {
        ...
        var new_total_credit_amount = parseFloat(total_credit_amount) + parseFloat(amount);
        if (total_debit_amount != '0') {
            if (new_total_credit_amount > total_debit_amount) {
                proceed = 0;
                $('#amount').css('border-color', '#dd4b39');
                $('#amount_label').css('color', '#dd4b39');
                $('#amount_help_block').html('<font color="#dd4b39">Total credit amount is increasing.</font>');
            } else {
                $('#amount').css('border-color', 'rgb(210, 214, 222)');
                $('#amount_label').css('color', 'black');
                $('#amount_help_block').html('');
    ...
    if (proceed != 0) {
        ...
        $('#ledger_name').val('');
        $('#amount').val('');
        $('#dr_val').attr('disabled', 'disabled');
        $('#cr_val').removeAttr('disabled');
        $('#entry_type').val('cr');
